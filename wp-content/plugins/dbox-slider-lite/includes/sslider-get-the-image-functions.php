<?php
/**
 * Almost all of the below code belongs to 'Get the Image' WordPress plugin by Justin Tadlock. This is the perfect code for extracting post images, I have just changed the function * names specific to DboxLite Slider, as someone would like to use the 'Get the Image' WordPress plugin separately. 
 ...
 * is found, call dboxlite_sslider_display_the_image() to format it.
 *
 * @since 0.1
 ...
 */
function dboxlite_sslider_get_the_image( $args = array() ) {
	global $post;
	...
	$args = apply_filters( 'dboxlite_sslider_get_the_image_args', $args );
	...
	extract( $args );
		...
		if ( $custom_key )
			$image = dboxlite_sslider_image_by_custom_field( $args );
 ...
 * @return array|bool
 */
function dboxlite_sslider_image_by_custom_field( $args = array() ) {
 ...
 * other filters are later added in the dboxlite_sslider_display_the_image() function.
 *
 * @since 0.4
 * @param array $args
 * @return array|bool
 */
function dboxlite_sslider_image_by_the_post_thumbnail( $args = array() ) {
 ...
 * @return array|bool
 */
function dboxlite_sslider_image_by_attachment( $args = array() ) {
...
/**
 * Scans the post for images within the content.  Not called by default with dboxlite_sslider_get_the_image().
 ...
 * @return array|bool
 */
function dboxlite_sslider_image_by_scan( $args = array() ) {
 ...
 * given in an array.  Not used with dboxlite_sslider_get_the_image() by default.
 *
 * @since 0.3
 * @param array $args
 * @return array
 */
function dboxlite_sslider_image_by_default( $args = array() ) {
 ...
 */
function dboxlite_sslider_display_the_image( $args = array(), $image = false ) {
	...
	if($timthumb=='1'){
	  $html = '<img src="'.dboxlite_slider_plugin_url( 'includes/timthumb.php' ) .'?src='. $image['url'] . '&amp;h='.$h.'&amp;w='.$w.'" alt="' . esc_attr( strip_tags( $image_alt ) ) . '" class="' . esc_attr( $class ) . '"' . $width . $height . $style .' />';	  
    }
	else{
	...
	if ( $link_to_post ) {
	    ...
	    if($permalink!='') {
			$image_title=($image_title_text=='1')?( 'title="' . esc_attr( apply_filters( 'the_title', get_post_field( 'post_title', $post_id ) ) ) . '" '):'';
			$html = '<a href="' . $permalink . '" '.$image_title.' '.$a_attr.'>' . $html . '</a>';
		}
	}
...
/**
 * Get the image with a link to the post.  Use dboxlite_sslider_get_the_image() instead.
 *
 * @since 0.1
 * @deprecated 0.3
 */
function dboxlite_sslider_get_the_image_link( $deprecated = '', $deprecated_2 = '', $deprecated_3 = '' ) {
	dboxlite_sslider_get_the_image();
}
