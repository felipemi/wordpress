...
<div id="container">
    <div id="content">
        ...
        <?php if (is_day()) : ?>
            <h2 class="page-title"><?php printf(__('Daily Archives: <span>%s</span>', 'cdeasy'), get_the_time(get_option('date_format'))) ?></h2>
        <?php elseif (is_month()) : ?>
            <h2 class="page-title"><?php printf(__('Monthly Archives: <span>%s</span>', 'cdeasy'), get_the_time('F Y')) ?></h2>
        <?php elseif (is_year()) : ?>
            <h2 class="page-title"><?php printf(__('Yearly Archives: <span>%s</span>', 'cdeasy'), get_the_time('Y')) ?></h2>
        <?php elseif (isset($_GET['paged']) && !empty($_GET['paged'])) : ?>
            <h2 class="page-title"><?php _e('Blog Archives', 'cdeasy') ?></h2>
        <?php endif; ?>
        ...
        if ($total_pages > 1) {
            ...
            <div id="nav-above" class="navigation">
                <div class="nav-previous"><?php next_posts_link(__('<span class="meta-nav">«</span> Older posts', 'cdeasy')) ?></div>
...
<?php while (have_posts()) : the_post(); ?>
            ...
            <div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
                <h2 class="entry-title"><a href="<?php the_permalink(); ?>" title="<?php printf(__('Permalink to %s', 'cdeasy'), the_title_attribute('echo=0')); ?>" rel="bookmark"><?php the_title(); ?></a></h2>
    ...
    <?php the_excerpt(__('Continue reading <span class="meta-nav">»</span>', 'cdeasy')); ?>
                ...
                <div class="entry-utility">
                    ...
                    <span class="comments-link"><?php comments_popup_link(__('Leave a comment', 'cdeasy'), __('1 Comment', 'cdeasy'), __('% Comments', 'cdeasy')) ?></span>
    <?php edit_post_link(__('Edit', 'cdeasy'), "<span class=\"meta-sep\">|</span>\n\t\t\t\t\t\t<span class=\"edit-link\">", "</span>\n\t\t\t\t\t\n") ?>
                </div><!-- #entry-utility -->
            </div><!-- #post-<?php the_ID(); ?> -->
